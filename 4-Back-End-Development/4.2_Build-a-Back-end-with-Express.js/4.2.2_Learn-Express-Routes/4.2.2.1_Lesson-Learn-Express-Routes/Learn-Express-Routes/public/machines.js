...
const activeMachineImage = {
  expressions: "./assets/svg/server-machine-active.svg",
  animals: "./assets/svg/server-machine-animals.svg",
};

const activeSingularRouteImage = {
  expressions: "./assets/svg/expression-route-active.svg",
  animals: "./assets/svg/animal-route-active.svg",
};

const activePluralRouteImage = {
  expressions: "./assets/svg/expressions-route-active.svg",
  animals: "./assets/svg/animals-route-active.svg",
};
  ...
  $.ajax("/", {
    success: function () {
      ...
      if (
        ...
        $("#expression-route").attr("src") ===
          "./assets/svg/expression-route-inactive.svg"
      ) {
        ...
        $("#expressions-route").attr(
          "src",
          activePluralRouteImage[currentMode]
        );
        activateExpressions();
      }
    },
    error: function (xhr) {
      if (xhr.status !== 404) {
        ...
        $("#expressions-route").attr(
          "src",
          "./assets/svg/expressions-route-inactive.svg"
        );
      }
    },
  });
...
function activateMachine() {
  ...
  $("#beaker").attr("src", "./assets/svg/beaker-active-neutral.svg");

  if ($("#lightbulb").attr("src") === "./assets/svg/lightbulb-inactive.svg") {
    $("#lightbulb").attr("src", "./assets/svg/lightbulb-active.svg");
  }
}
...
function deactivateMachine() {
  ...
  $("#server-machine").attr("src", "./assets/svg/server-machine-inactive.svg");
  $("#lightbulb").attr("src", "./assets/svg/lightbulb-inactive.svg");
  $("#beaker").attr("src", "./assets/svg/beaker-inactive.svg");
  $("#code-number").text("");
...
function triggerExpressionRequest() {
  ...
  $("#expression-route").attr(
    "src",
    "./assets/svg/expression-route-pending.svg"
  );
...
function triggerExpressionsRequest() {
  ...
  $("#expressions-route").attr(
    "src",
    "./assets/svg/expressions-route-pending.svg"
  );
  $("#code-number").text("");
  $("#code-description").text("");
  clearExpressionText();
...
function animateMachine(animationDelay) {
  $("#beaker").attr("src", "./assets/svg/beaker-pending.svg");
  $("#lightbulb").attr("src", "./assets/svg/lightbulb-pending.svg");
  $("#sliders")
    .queue(function (next) {
      $(this).attr("src", "./assets/svg/sliders-active-1.svg");
      next();
    })
    .delay(animationDelay)
    .queue(function (next) {
      $(this).attr("src", "./assets/svg/sliders-active-2.svg");
      next();
    })
    .delay(animationDelay)
    .queue(function (next) {
    ...
    .delay(animationDelay)
    .queue(function (next) {
      $(this).attr("src", "./assets/svg/sliders-inactive.svg");
      next();
    });
  $("#buttons")
    .queue(function (next) {
      $(this).attr("src", "./assets/svg/buttons-active-1.svg");
      next();
    })
    .delay(animationDelay)
    .queue(function (next) {
      $(this).attr("src", "./assets/svg/buttons-active-2.svg");
      next();
    })
    .delay(animationDelay)
    .queue(function (next) {
    ...
    .delay(animationDelay)
    .queue(function (next) {
      $(this).attr("src", "./assets/svg/buttons-inactive.svg");
      next();
    });
}
...
function animateGoodExpressionRequest(statusCode, statusDescription, response) {
  ...
  $("#expression-route").attr("src", "./assets/svg/expression-route-green.svg");
...
function animateGoodExpressionsRequest(
  statusCode,
  statusDescription,
  response
) {
  ...
  $("#expressions-route").attr(
    "src",
    "./assets/svg/expressions-route-green.svg"
  );
...
function animateGoodRequest(statusCode, statusDescription) {
  $("#lightbulb").attr("src", "./assets/svg/lightbulb-green.svg");
...
function animateBadExpressionRequest(statusCode, statusDescription) {
  ...
  $("#expression-route").attr("src", "./assets/svg/expression-route-red.svg");
}

function animateBadExpressionsRequest(statusCode, statusDescription) {
  animateBadRequest(statusCode, statusDescription);
  $("#expressions-route").attr("src", "./assets/svg/expressions-route-red.svg");
}

function animateBadRequest(statusCode, statusDescription) {
  $("#lightbulb").attr("src", "./assets/svg/lightbulb-red.svg");
